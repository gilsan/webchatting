...
export class RequestService {
  ...
  */
  ...
  addFriendSub2(email: string, id: string): Observable<any> {
    return from(this.friendsRef.doc(id).collection('myfriends').add({ email }));
  }


  // 기존에 콜렉션이 있는 경우
  addFriendWhenNoExist(email, uid): Observable<any> {
    this.approveFriend$.next(true);
    return from(this.db.doc(`friends/${uid}`).collection('myfriends').add({ email }));
  }
